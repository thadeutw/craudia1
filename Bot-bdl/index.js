...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = ` @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* `
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '❬❗❭ APENAS EM GRUPO',
					ownerG: '❬❗❭ SÓ PROPRIETÁRIO ',
					ownerB: '❬❗❭  SÓ PROPRIETÁRIO ',
					admin: '❬❗❭ SOMENTE ADEMIRO ',
					Badmin: '❬❗❭ O BOT DEVE SER ADMINISTRADOR '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5513997758115@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    break
                case 'ssweb':
					if (args.length < 1) return reply('Cadê o url viado?')
					teks = body.slice(7)
					reply(mess.wait)
			...
			case 'loli': 
				    try {
						res = await fetchJson(`https://neko.life/random`, {method: 'get'})
						buffer = await getBuffer(res.url)
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('𝙏𝙧𝙖𝙣𝙨𝙢𝙞𝙨𝙨𝙖̃𝙤 𝙛𝙚𝙞𝙩𝙖 𝙘𝙤𝙢 𝙨𝙪𝙘𝙚𝙨𝙨𝙤 ')
					} else {
						for (let _ of anu) {
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativado com sucesso o comando Welcome / para desativar digite: welcome 0')
					} else if (Number(args[0]) === 0) {
